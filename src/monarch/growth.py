...
def fg_isotropic_goktepe(model, f_g_old, dt):
    ...
    # Prevent growth beyond min and max values, can occur if time step is too big
    f_g = np.clip(f_g.T, [model.growth.theta_f_min ** (1.0 / 3.0), model.growth.theta_f_min ** (1.0 / 3.0),
                          model.growth.theta_f_min ** (1.0 / 3.0)],
                  [model.growth.theta_f_max ** (1.0 / 3.0), model.growth.theta_f_max ** (1.0 / 3.0),
                   model.growth.theta_f_max ** (1.0 / 3.0)]).T
...
def fg_isotropic_work(model, f_g_old, dt):
    ...
    # Prevent growth beyond min and max values, can occur if time step is too big
    f_g = np.clip(f_g.T, [model.growth.theta_f_min ** (1.0 / 3.0), model.growth.theta_f_min ** (1.0 / 3.0), model.growth.theta_f_min ** (1.0 / 3.0)],
                  [model.growth.theta_f_max ** (1.0 / 3.0), model.growth.theta_f_max ** (1.0 / 3.0), model.growth.theta_f_max ** (1.0 / 3.0)]).T
...
def fg_hybrid_work(model, f_g_old, dt):
    """Hybrid growth law between the Witzenburg and Jones & Oomen laws: sigmoid growth rate function with growth limiter
    based on total growth - isotropic version with work as the growth driver"""
